...
public class SimilarityMetadata {
  ...
  * @throws IOException if the JSON Object is invalid with respect to SimilarityMetadata
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!SimilarityMetadata.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SimilarityMetadata is not found in the empty JSON string", SimilarityMetadata.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!SimilarityMetadata.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SimilarityMetadata` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : SimilarityMetadata.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SimilarityMetadata.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SimilarityMetadata' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SimilarityMetadata> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SimilarityMetadata.class));

       return (TypeAdapter<T>) new TypeAdapter<SimilarityMetadata>() {
           @Override
           public void write(JsonWriter out, SimilarityMetadata value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SimilarityMetadata read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of SimilarityMetadata given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of SimilarityMetadata
  * @throws IOException if the JSON string is invalid with respect to SimilarityMetadata
  */
  public static SimilarityMetadata fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SimilarityMetadata.class);
  }

 /**
  * Convert an instance of SimilarityMetadata to an JSON string
  *
  * @return JSON string
  */
